define(['feature_computation',
        'mocha',
        'chai'],
    function(feature_computation, mocha, chai) {
        describe("Computing log ratios", function() {
            it("Computes the natural logarithm (i.e. using e as a base)", function() {
                ...
                chai.assert.equal(feature_computation.computeBalance(1, Math.E), -1);
            });
            ...
            it("Computes the correct log ratio given valid inputs", function() {
                chai.assert.approximately(feature_computation.computeBalance(2, 5), -0.91629, delta);
                chai.assert.approximately(feature_computation.computeBalance(5, 2), 0.91629, delta);
                chai.assert.approximately(feature_computation.computeBalance(500, 400), 0.22314, delta);
                chai.assert.equal(feature_computation.computeBalance(1.5, 0.5), Math.log(3));
                chai.assert.equal(feature_computation.computeBalance(5, 5), 0);
                chai.assert.equal(feature_computation.computeBalance(1, 1), 0);
                chai.assert.equal(feature_computation.computeBalance(Math.E, Math.E), 0);
            });
            it("Throws an error when passed non-numerical input(s)", function() {
                ...
                chai.assert.throws(function() { feature_computation.computeBalance('2', 5); });
                chai.assert.throws(function() { feature_computation.computeBalance(2, '5'); });
                chai.assert.throws(function() { feature_computation.computeBalance('3', '4'); });
                chai.assert.throws(function() { feature_computation.computeBalance([1, 2]); });
            });
            it("Throws an error when passed < 2 arguments", function() {
                chai.assert.throws(function() { feature_computation.computeBalance(3); });
                chai.assert.throws(function() { feature_computation.computeBalance(); });
                chai.assert.throws(function() { feature_computation.computeBalance([1, 2]); });
            });
        });
    }
